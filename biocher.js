var geometry = /* color: #d63000 */ee.Geometry({
  ...
  ee.Geometry.Polygon(
      [[[0.14023444040608624, 52.2995545949096],
        ...
        [0.20718237741780499, 52.2995545949096]]], null, false);
...
var sentinel = sentinel.map(cloudmask)
...
var table = table.filter(ee.Filter.metadata('ADM0_CODE','equals',256))

//Load COPERNICUS LANCOVER dataset
var copernicus = ee.ImageCollection("COPERNICUS/Landcover/100m/Proba-V-C3/Global")
var copernicus = copernicus.select('crops-coverfraction').mosaic().clip(table)
...
var slope = slope.clip(table)
Map.addLayer(slope,{min:0,max:10},'Slope (degrees)',0)

//Mask slope above 10°
var slope_mask10 = slope.gt(10)
var slope_mask10 = slope_mask10.mask(slope_mask10)
...
var area_211 = ee.Number(areaImage.reduceRegion({
...
var areaImage_p = mask2.multiply(ee.Image.pixelArea());
var area_212 = ee.Number(areaImage_p.reduceRegion({
reducer: ee.Reducer.sum(),
geometry: table,
scale: 100,
maxPixels: 1e10
...
var areaImage2 = slope_mask10.multiply(ee.Image.pixelArea());
var areaslope = ee.Number(areaImage2.reduceRegion({
reducer: ee.Reducer.sum(),
geometry: table,
scale:30,
maxPixels: 1e10
...
var mask_land_slope = mask.eq(1).and(slope_mask10.eq(1))
var mask_land_slope = mask_land_slope.mask(mask_land_slope)
Map.addLayer(mask_land_slope,{palette:['orange']},'Mask arable land and slope >10°',0)

//Calculate area for above mask
var areaImage3 = mask_land_slope.multiply(ee.Image.pixelArea());
var totalarea = ee.Number(areaImage3.reduceRegion({
reducer: ee.Reducer.sum(),
geometry: table,
scale: 30,
maxPixels: 1e10
...
var crop_mask = copernicus.gte(80)
var crop_mask = crop_mask.mask(crop_mask)
Map.addLayer(crop_mask,{palette:'blue'},'Crop cover fraction >80%',0)

//Mask slope greater than 5°
var slope_mask5 = slope.gte(5)
var slope_mask5 = slope_mask5.mask(slope_mask5)
Map.addLayer(slope_mask5,{palette:'purple'},'Slope higher than 5°',0)

//Mask crop cover +80% and slope +5°
var cc_slope = crop_mask.eq(1).and(slope_mask5.eq(1))
var cc_slope = cc_slope.mask(cc_slope)
Map.addLayer(cc_slope,{palette:'red'},'Mask crop cover >80% and slope >5°',0)

//Calculate area for above mask
var areaImage4 = cc_slope.multiply(ee.Image.pixelArea());
var totalarea4 = ee.Number(areaImage4.reduceRegion({
reducer: ee.Reducer.sum(),
geometry: table,
scale: 30,
maxPixels: 1e10
...
//Filter BSI dataset
var bsi_june = bsi.filterBounds(table)
.filterDate('2020-6-1','2020-7-1')
.filter(ee.Filter.metadata('CLOUDY_PIXEL_PERCENTAGE','less_than',9))
.mosaic()
.clip(table)
...
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',1)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI Jan 2020')
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',2)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI Feb 2020')
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',3)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI Mar 2020')
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',4)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI Apr 2020')
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',5)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI May 2020')
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',6)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI Jun 2020')
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',7)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI Jul 2020')
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',8)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI Aug 2020')
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',9)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI Sep 2020')
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',10)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI Oct 2020')
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',11)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI Nov 2020')
Map.addLayer(byMonthYear.filter(ee.Filter.metadata('month','equals',12)),{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI Dec 2020')


//------------------------------------------MEAN NDVI PER MONTH 2020-----------------------------------
//ADD MSAVI "BAND" TO SENTINEL DATASET 
var msavi = sentinel.map(function(image) {
return image.select().addBands(image.expression(
...
var byMonthYear_msavi = ee.ImageCollection.fromImages(
years.map(function(y) {
  return months.map(function (m) {
    return msavi2
      .filter(ee.Filter.calendarRange(y, y, 'year'))
...
var byMonthYear_msavi = byMonthYear_msavi
.map(function(image){return image.clip(table)}) ;
print('Mean MSAVI 2020',byMonthYear_msavi)

//Map.addLayer(byMonthYear,{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI per month -year 2020-')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',1)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI Jan 2020')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',2)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI Feb 2020')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',3)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI Mar 2020')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',4)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI Apr 2020')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',5)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI May 2020')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',6)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI Jun 2020')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',7)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI Jul 2020')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',8)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI Aug 2020')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',9)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI Sep 2020')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',10)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI Oct 2020')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',11)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI Nov 2020')
Map.addLayer(byMonthYear_msavi.filter(ee.Filter.metadata('month','equals',12)),{min:-1,max:1,palette:['#a6611a','#dfc27d','#f5f5f5','#80cdc1','#018571']},'Mean MSAVI Dec 2020')

//------------------------------------------------MEAN MSAVI PER MONTH 2020---------------------------
//ADD NDVI "BAND" TO SENTINEL DATASET 
var ndvi_cloudy = sentinel.map(function(image) {
return image.select().addBands(image.normalizedDifference(['B8','B4'])).rename('NDVI');
});

var ndvi_cloudy2 = ndvi_cloudy
.filterDate('2020-1-1','2020-12-31')
.filterBounds(table)
//.filter(ee.Filter.metadata('CLOUDY_PIXEL_PERCENTAGE','less_than',10))
//print(bsi2)
...
var byMonthYear_ndvi = ee.ImageCollection.fromImages(
years.map(function(y) {
  return months.map(function (m) {
    return ndvi_cloudy2
      .filter(ee.Filter.calendarRange(y, y, 'year'))
...
var byMonthYear_ndvi = byMonthYear_ndvi
.map(function(image){return image.clip(table)}) ;
print('Mean NDVI 2020',byMonthYear_ndvi)

//Map.addLayer(byMonthYear,{min:-0.3,max:0.2,palette:['red','orange','yellow','green']},'Mean BSI per month -year 2020-')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',1)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI Jan 2020')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',2)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI Feb 2020')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',3)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI Mar 2020')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',4)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI Apr 2020')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',5)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI May 2020')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',6)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI Jun 2020')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',7)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI Jul 2020')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',8)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI Aug 2020')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',9)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI Sep 2020')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',10)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI Oct 2020')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',11)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI Nov 2020')
Map.addLayer(byMonthYear_ndvi.filter(ee.Filter.metadata('month','equals',12)),{min:-1,max:1,palette:['#f7fcf5','#caeac3','#7bc87c','#2a924a','#00441b']},'Mean NDVI Dec 2020')

//-----------------------------------------------------------------------------------------------------
//BSI masking (values greater than 0.1)
var bsi_mask = bsi_june.gt(0.1)
var bsi_mask = bsi_mask.mask(bsi_mask)
Map.addLayer(bsi_mask,{palette:'black'},'BSI Mask +0.1',0)

//BSI masking (values greater than 0.05)
var bsi_mask2 = bsi_june.gt(0.05)
var bsi_mask2 = bsi_mask2.mask(bsi_mask2)
Map.addLayer(bsi_mask2,{palette:'olive'},'BSI Mask +0.05',0)

//Soil Moisture Index calculation (0.9)
var msi = sen.normalizedDifference(['B8_median','B11_median'])
Map.addLayer(ndvi,{min:-1,max:1,palette:['red','yellow','green']},'NDMI',0)

//NDMI Masking (0.5)
var msi_mask = msi.gt(0.5)
var msi_mask = msi_mask.mask(msi_mask)
Map.addLayer(msi_mask,{palette:'yellow'},'NDMI Mask +0.5',0)

//BSI +0.1 and Slope +5° mask
var bsi_slope = bsi_june.gt(0.1).and(slope.gt(5))
var bsi_slope = bsi_slope.mask(bsi_slope)
...
//January
var precipitation_jan = precipitation.select('IRprecipitation')
.filterDate('2021-1-1','2021-2-1')

var pp_acum_jan = precipitation_jan
...
//February
var precipitation_feb = precipitation.select('IRprecipitation')
.filterDate('2021-2-1','2021-3-1')

var pp_acum_feb = precipitation_feb
.reduce(ee.Reducer.sum())
print(pp_acum_feb)
...
//March
var precipitation_march = precipitation.select('IRprecipitation')
.filterDate('2021-3-1','2021-4-1')

var pp_acum_march = precipitation_march
.reduce(ee.Reducer.sum())
print(pp_acum_march)
...
//April
var precipitation_apr = precipitation.select('IRprecipitation')
.filterDate('2021-4-1','2021-5-1')

var pp_acum_apr = precipitation_apr
.reduce(ee.Reducer.sum())
print(pp_acum_apr)
...
//May
var precipitation_may = precipitation.select('IRprecipitation')
.filterDate('2021-5-1','2021-6-1')

var pp_acum_may = precipitation_may
.reduce(ee.Reducer.sum())
print(pp_acum_may)
...
//June
var precipitation_jun = precipitation.select('IRprecipitation')
.filterDate('2021-6-1','2021-7-1')

var pp_acum_jun = precipitation_jun
.reduce(ee.Reducer.sum())
print(pp_acum_jun)
...
//July
var precipitation_jul = precipitation.select('IRprecipitation')
.filterDate('2021-7-1','2021-8-1')

var pp_acum_jul = precipitation_jul
.reduce(ee.Reducer.sum())
print(pp_acum_jul)
...
//August
var precipitation_aug = precipitation.select('IRprecipitation')
.filterDate('2021-8-1','2021-9-1')

var pp_acum_aug = precipitation_aug
.reduce(ee.Reducer.sum())
print(pp_acum_aug)
...
//September
var precipitation_sep = precipitation.select('IRprecipitation')
.filterDate('2021-9-1','2021-10-1')

var pp_acum_sep = precipitation_sep
.reduce(ee.Reducer.sum())
print(pp_acum_sep)
...
//October
var precipitation_oct = precipitation.select('IRprecipitation')
.filterDate('2021-10-1','2021-11-1')

var pp_acum_oct = precipitation_oct
.reduce(ee.Reducer.sum())
print(pp_acum_oct)
...
//Mean SSM JAN 2020
//var ssm1 = ssm.filterDate('2020-1-1','2020-1-31')
//var ssm1 = ssm1.reduce(ee.Reducer.mean())
...
//Mean SSM FEB 2020
//var ssm2 = ssm.filterDate('2020-2-1','2020-2-29')
//var ssm2 = ssm2.reduce(ee.Reducer.mean())
...
//Mean SSM MAR 2020
//var ssm3 = ssm.filterDate('2020-3-1','2020-3-31')
//var ssm3 = ssm3.reduce(ee.Reducer.mean())
...
//Mean SSM APR 2020
//var ssm4 = ssm.filterDate('2020-4-1','2020-4-30')
//var ssm4 = ssm4.reduce(ee.Reducer.mean())
...
//Mean SSM MAY 2020
//var ssm5 = ssm.filterDate('2020-5-1','2020-5-31')
//var ssm5 = ssm5.reduce(ee.Reducer.mean())
...
//Mean SSM JUN 2020
//var ssm6 = ssm.filterDate('2020-6-1','2020-6-30')
//var ssm6 = ssm6.reduce(ee.Reducer.mean())
...
//Mean SSM JUL 2020
//var ssm7 = ssm.filterDate('2020-7-1','2020-7-31')
//var ssm7 = ssm7.reduce(ee.Reducer.mean())
...
//Mean SSM AUG 2020
//var ssm8 = ssm.filterDate('2020-8-1','2020-8-31')
//var ssm8 = ssm8.reduce(ee.Reducer.mean())
...
//Mean SSM SEP 2020
//var ssm9 = ssm.filterDate('2020-9-1','2020-9-30')
//var ssm9 = ssm9.reduce(ee.Reducer.mean())
...
//Mean SSM OCT 2020
//var ssm10 = ssm.filterDate('2020-10-1','2020-10-31')
//var ssm10 = ssm10.reduce(ee.Reducer.mean())
...
//Mean SSM NOV 2020
//var ssm11 = ssm.filterDate('2020-11-1','2020-11-30')
//var ssm11 = ssm11.reduce(ee.Reducer.mean())
...
//Mean SSM DEC 2020
//var ssm12 = ssm.filterDate('2020-12-1','2020-12-31')
//var ssm12 = ssm12.reduce(ee.Reducer.mean())
...
var ssm_avg = ssm.reduce(ee.Reducer.mean())
...
.clip(table);
var visualization_soil_texture = {
min: 1.0,
max: 12.0,
palette: [
...
var palette = result['b0' + "_class_palette"];
var names = result['b0' + "_class_names"];
...
var mask_bsi_ssm_may = (byMonthYear.filter(ee.Filter.metadata('month','equals',5)).mosaic()).gt(0.01)//
.and((ssm.filterDate('2020-5-1','2020-5-31').reduce(ee.Reducer.mean())).lt(12))
var mask_bsi_ssm_may = mask_bsi_ssm_may.mask(mask_bsi_ssm_may)
...
var maskmaysep = mask_bsi_ssm_may.eq(1)
.and((byMonthYear.filter(ee.Filter.metadata('month','equals',9)).mosaic()).gt(0.01))
.and((ssm.filterDate('2020-9-1','2020-9-30').reduce(ee.Reducer.mean())).lt(10))
var maskmaysep = maskmaysep.mask(maskmaysep)
...
var mask_may_sep_class = maskmaysep.eq(1).and((soil_texture.select('b0')).gt(6))
var mask_may_sep_class = mask_may_sep_class.mask(mask_may_sep_class)
